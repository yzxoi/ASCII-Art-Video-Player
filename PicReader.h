...
public:
	...
	UINT getWidth() const { return imageWidth; }
	UINT getHeight() const { return imageHeight; }
private:
	void init();
...
PicReader::PicReader() : m_pConvertedSourceBitmap(nullptr), m_pIWICFactory(nullptr), imageWidth(0), imageHeight(0) {
	init();
}
...
Array PicReader::getData() {
	...
	******************************************************************/
	...
	Array rgbaArray(x * y * 4);
	
	int* arrayData = rgbaArray.get_content();
	for (UINT i = 0; i < x * y * 4; ++i) {
		arrayData[i] = (int)data[i];
	}
	
	rgbaArray.reshape((int)y, (int)x, 4);
